...
class Formatter(base.BaseDocHandler):
    def run(self, sess: base.Session):
        ...
        # ===== Step 1: 调用 Claude LLM 分块 =====
        client = Anthropic(
            api_key=os.getenv("ANTHROPIC_API_KEY")
        )

        print("🧠 正在调用 Claude 模型进行语义分块 ...")

        response = client.messages.create(
            model=os.getenv("CLAUDE_MODEL"),  # 如 "claude-3-sonnet-20240229"
            max_tokens=4096,
            temperature=0.1,
            system="你是一个知识文档结构分析助手。请将文本划分为若干语义完整的逻辑单元，每个块应表达相对独立的主题。请检测并提取文本中出现的手机号。请严格输出指定的JSON格式。",
            messages=[{
                "role": "user",
    ...
    def get_prompt(self, content: str) -> str:
        ...
        response_format_str = json.dumps(response_format, ensure_ascii=False, indent=2)

        return f"""
请将以下文本划分为若干语义完整的逻辑单元（即段落级知识块），每个块应表达相对独立的主题。请检测并提取文本中出现的手机号。
